...
export default async function ProfessionalResourcesPage() {
  ...
  const resources = (Array.isArray(all) ? all : []).filter((r) => r.client === false);

  return (
    <main className="container mx-auto px-6 lg:px-20 py-16">
      <h1 className="text-3xl font-bold mb-2">Professional Resources</h1>
      <p className="text-gray-600 mb-8">
        Referral info, clinician resources, and professional documents.
      </p>

      {resources.length === 0 ? (
        <p className="text-gray-700">No professional resources available yet.</p>
      ) : (
